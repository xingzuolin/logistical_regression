...
def chi2_cut(df, col, max_groups=None, threshold=None):
    ...
    # freq_tab = freq_tab.ix[:, [good_name, bad_name]]
...
def bin_char_name(x,data_df,var_name):
    bin_t = ','.join([data_df.ix[i, var_name] for i in range(x[0], x[1]+1)])
    return bin_t


def bin_num_name(x, data_df, var_name):
    bin_list = []
    x_sorted = sorted(x, key=lambda y: y[0])
    x_len = len(x_sorted)
    for i, term in enumerate(x_sorted):
        if i == 0:
            bin_str = '[0,' + str(data_df.ix[term[1], var_name]) + ']'
            bin_list.append(bin_str)
        elif i < x_len-1:
            bin_str = '(' + str(data_df.ix[x_sorted[i-1][1], var_name]) + ',' + str(data_df.ix[term[1], var_name]) + ']'
            bin_list.append(bin_str)
        elif i == x_len-1:
            bin_str = '(' + str(data_df.ix[x_sorted[i-1][1], var_name]) + ',--)'
            bin_list.append(bin_str)
    return bin_list
...
def best_df(date_df, items, na_df, factor_name, bad_name, good_name,total_all,good_all,bad_all):
    df0 = pd.DataFrame()
    if items:
        if str(date_df[factor_name].dtype).find('object') >= 0:
            piece0 = list(map(lambda x: '(' + bin_char_name(x,date_df,factor_name) + ')', items))
            dt_type = 'STRING'
        else:
            piece0 = bin_num_name(items, date_df, factor_name)
            dt_type = 'NUMBER'
        ...
        var_name_list = [str(factor_name)]*len(piece0)
        dt_type_list = [dt_type]*len(piece0)
        df0 = pd.DataFrame(list(zip(var_name_list,dt_type_list,piece0,total0,bad0,good0,total_per0,bad_rate0,good_rate0,good_per0,bad_per0)),columns=['varaible','type',factor_name,'Total_Num','Bad_Num','Good_Num','Total_Pcnt','Bad_Rate','Good_Rate','Good_Pcnt','Bad_Pcnt'])
        try:
            ...
            df0.sort_values(by='Bad_Rate', ascending=True, inplace=True)
...
def df_pivot_two(data_df, var_name, target, good_name, bad_name, type):
    if len(data_df) == 0:
    ...
    if type == 'CHAR':
        ...
        date_df.sort_values(by='bad_rate', ascending=True, inplace=True)
        date_df.drop('bad_rate', axis=1, inplace=True)
...
def find_best_bin(path='', data=pd.DataFrame(), sep=',', subset=[0, 1], y=None,var_name=None,groups=5,rate=0.05):
    ...
    if len(data) == 0:
        ...
        return data
    col_list = convert_lower(list(data.columns))
    y = convert_lower(y)
    var_name = convert_lower(var_name)
    ...
    # type_len = len(set([type(k) for k in list(data_df_notnull[var_name])]))
    if str(data_df_notnull[var_name].dtype).find('object') < 0:
        data_df = df_pivot_two(data_df_notnull, var_name, y, good_name, bad_name,type='NUM')
        if len(data_df_null) > 0:
            na_df = df_null_group(data_df_null,var_name,y,good_name,bad_name, missing_fill=-999)
        else:
            na_df = pd.DataFrame()
    if str(data_df_notnull[var_name].dtype).find('object') >= 0:
        data_df = df_pivot_two(data_df_notnull, var_name, y, good_name, bad_name,type='CHAR')
        if len(data_df_null) > 0:
            na_df = df_null_group(data_df_null, var_name, y, good_name, bad_name, missing_fill='_null_')
        else:
            na_df = pd.DataFrame()
    if len(data_df) == 0:
        return data_df
    ...
    print('spend time(s):', round(time.time() - start_time, 0))
    return bin_df
